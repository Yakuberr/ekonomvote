...
class VoteForm(forms.Form):
    ...
    def clean(self):
        ...
        try:
            ...
            raise ValidationError(f"Kandydatura o id: {candidate_registration_id} nie istnieje")
        voting_id = registration_obj.voting.id
        if self.expected_voting.id != voting_id:
            raise ValidationError(f"Kandydatura o id: {candidate_registration_id} nie należy do obecnego głosowania")
...
class BaseVoteFormSet(BaseFormSet):
    ...
    def clean(self):
        ...
        if self.total_form_count() != expected_vote_count:
            raise ValidationError(f"Nie oddano wymaganej liczby głosów: {self.total_form_count()}/{expected_vote_count}")
